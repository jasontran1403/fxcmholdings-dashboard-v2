...
const FloatingLabels = () => {
	...
	return (
		<Card className="mt-2">
			<Card.Header>
				<h4 className="header-title">Tạo lệnh rút tiền</h4>
			</Card.Header>
			<Card.Body>
				<Row>
					...
					<Col lg={12} className="mb-2">
						<FloatingLabel
							...
							label="Địa chỉ ví cá nhân"
							className="mb-3"
						>
							<Form.Control type="text" placeholder="Nhập địa chỉ ví cá nhân" />
						</FloatingLabel>
					</Col>
					<Col lg={12} className="mb-2">
						<FloatingLabel
							...
							className="mb-3"
						>
							<Form.Control type="number" min="0" placeholder="Nhập số tiền nạp" />
						</FloatingLabel>
					</Col>
...
const DataTables = () => {
	...
	useEffect(() => {
		let configCommissionHistory = {
			method: "get",
			url: `${url}/api/getTransaction/${username}/Withdraw`,
			headers: {
		...
		<>
			...
			<Row className="mt-2">
				<Col>
					<Card>
						<Card.Header>
							<h4 className="header-title">Lịch sử rút tiền</h4>
						</Card.Header>
						<Card.Body>
							<Table<Employee>
								columns={columns}
